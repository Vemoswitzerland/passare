-- ════════════════════════════════════════════════════════════════════
-- passare.ch — Hotfix: anonymitaet_level enum-Cast in submit_inserat_step
-- ════════════════════════════════════════════════════════════════════
-- Cyrill: «CASE types anonymitaet_level and text cannot be matched»
--
-- anonymitaet_level ist ein Enum (public.anonymitaet_level), aber
-- p_data->>'...' liefert text. CASE-Branches müssen gleichen Type haben.
-- ════════════════════════════════════════════════════════════════════
...
begin
  ...
  if p_step = 2 then
    ...
    update public.inserate set
      ...
      anonymitaet_level = case when p_data ? 'anonymitaet_level'
        then nullif(p_data->>'anonymitaet_level','')::public.anonymitaet_level
        else anonymitaet_level end,
