'use client';

import { startGoogleOAuthAction } from '@/app/auth/actions';
import { cn } from '@/lib/utils';

type Props = {
  label?: string;
  className?: string;
};

/**
 * Sign in / Sign up with Google.
 * Triggert eine Server-Action die zur Supabase-OAuth-URL redirected.
 */
export function GoogleButton({ label = 'Mit Google fortfahren', className }: Props) {
  return (
    <form action={startGoogleOAuthAction} className={className}>
      <button
        type="submit"
        className={cn(
          'w-full inline-flex items-center justify-center gap-3 font-sans font-medium',
          'border border-stone rounded-soft px-6 py-3 text-[0.9375rem] text-ink',
          'bg-paper hover:bg-cream/40 hover:border-bronze/40',
          'transition-all duration-300 ease-out-expo',
          'disabled:opacity-50 disabled:pointer-events-none',
        )}
      >
        <GoogleIcon className="w-[18px] h-[18px]" />
        {label}
      </button>
    </form>
  );
}

function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} aria-hidden="true">
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}

export function AuthDivider({ children = 'oder' }: { children?: string }) {
  return (
    <div className="flex items-center gap-3 my-5">
      <span className="flex-1 h-px bg-stone" />
      <span className="font-mono text-[10px] uppercase tracking-widest text-quiet">{children}</span>
      <span className="flex-1 h-px bg-stone" />
    </div>
  );
}
