 ...
 * Erstellt das Käufer-Basic-Konto (V1 mock) und redirected zurück aufs Inserat.
 *
 * Wird vom Server-Component-Wrapper page.tsx in <SiteHeader/> + <SiteFooter/>
 * gewrapped — der Direkt-Import von page.tsx hier hätte die Homepage in den
 * Client-Bundle gezogen und ihren `metadata`-Export gebrochen.
 */

export function PasswortClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Redirect zurück aufs Inserat mit Erfolgs-Flag
    router.push(`/inserat/${listingId}?anfrage=ok`);
  }

  // Wenn keine listing-ID kommt, gehört das nicht zu einem Anfrage-Flow → Hinweis
  if (!listingId || !email) {
    return (
      <Section>
        <Container>
          <div className="max-w-md mx-auto text-center py-16">
            <h1 className="font-serif text-head-lg text-navy font-normal mb-3">
              Verifizierungs-Link ungültig
            </h1>
            <p className="text-body-sm text-muted leading-relaxed mb-6">
              Bitte starten Sie die Anfrage neu auf der gewünschten Inserat-Seite.
            </p>
            <Link
              href="/"
            ...
            <Reveal>
              <div className="text-center mb-8">
                ...
                <p className="text-body-sm text-muted leading-relaxed">
                  {name && <>Hallo <span className="text-navy">{name}</span> — </>}
                  vergeben Sie ein Passwort, um Ihr Käufer-Basic-Konto zu aktivieren.
                  Erst dann geht die Anfrage an den Verkäufer.
                </p>
                <p className="font-mono text-caption text-quiet mt-3 break-all">{email}</p>
              </div>
            </Reveal>
